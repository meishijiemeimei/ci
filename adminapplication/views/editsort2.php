...
<table width="1024" height="100%" bgcolor="#F5F5F5">
  ...
  <tr>
    ...
    <td  bgcolor="#FFFFFF" >
      ...
      <?foreach ($sort as $row):?>
        <form id="addshop" name="addshop" action="<?=base_url()?>admin.php/shop/editsort_update2/<?=$row->shop_sort_id2?>" method="post">
        分类名称： <input name="shop_sort_title2" type="text" id="shop_sort_title2" value="<?=$row->shop_sort_title2?>" />
        <br>
        类别： 按属性分类
       	<input type="hidden" name="sort_id2" id="sort_id2" value="2" />
